...
fn ssh_fixture_parses_hosts_options_and_commands() {
    let hosts = parse_ssh_config(include_str!("../../../tests/fixtures/ssh/config.sample"), 22);
    ...
    assert_eq!(
        prod.identity_file.as_ref().map(|path| path.display().to_string()),
        Some("~/.ssh/prod_web".to_string())
    );
    assert!(prod.options.contains(&"ProxyJump bastion".to_string()));
    assert!(prod
        .options
        .contains(&"ServerAliveInterval 30".to_string()));
    assert_eq!(
        prod.build_command(),
        vec![
            "ssh",
...
fn snippets_fixture_exercises_variables_search_and_sorting() {
    let config: SnippetsConfig =
        serde_json::from_str(include_str!("../../../tests/fixtures/snippets/sample-snippets.json"))
    ...
    assert_eq!(config.tags(), vec!["branch", "container", "docker", "git", "remote", "ssh"]);
    ...
    assert_eq!(variables.len(), 1, "duplicate variables should be collapsed");
    assert_eq!(variables[0].name, "branch_name");
    assert_eq!(
        variables[0].hint.as_deref(),
        Some("short branch name")
    );
    ...
    let ssh = config.find("ssh-connect").expect("ssh snippet should exist");
    assert_eq!(
    ...
    let by_usage: Vec<&str> = config.by_usage().into_iter().map(|s| s.id.as_str()).collect();
    assert_eq!(by_usage, vec!["git-feature-branch", "docker-run-port", "ssh-connect"]);
